...
export async function POST(request: Request) {
    try {
        ...
        const rowsWithUser = data.map((row: any) => ({
            ...row,
            user_id: user.id,
            // Common sales headers mapping
            sale_amount: parseFloat(row.sale_amount || row.revenue || row.amount || row.sales || 0),
        ...
        return NextResponse.json({
            success: true,
            message: `Successfully uploaded ${rowsWithUser.length} rows to sales.`
        });
